...
export const WishMaker = () => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    await navigator.clipboard.writeText(`${window.location.href}#/w?n=${encodeURI(inputValue)}`).catch((err) => {
      alert(err)
    }).finally(() => {
        ...
        <div>
          ...
          <form className="mt-16 w-full mx-auto" onSubmit={handleSubmit}>
            <div className="group relative w-72 md:w-80 lg:w-96 mx-auto">
              ...
              <input value={inputValue} onChange={(e: ChangeEvent<HTMLInputElement>) => setInputValue(e.target.value)} type="text" autoFocus className="peer h-10 w-full py-2 font-semibold rounded-xl bg-gray-50 px-4 outline-none border-2 transition-all duration-200 ease-in-out focus:bg-white focus:ring-2 focus:ring-red-500 focus:border-0" />
            </div>
